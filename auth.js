...
const OtpSchema = new mongoose.Schema({
  ...
  purpose: { type: String, enum: ['verify_register', 'verify_login', 'reactivate', 'reset_password'], required: true },
...
const updateReferralActivity = async (clientId) => {
  return await Referral2.findOneAndUpdate(
...
router.post('/register', async (req, res) => {
  try {
    ...
    res.status(201).json({
      success: true,
      userId: user._id,
      phone: maskPhone(cleanEmail),
      managerName: (await User.findById(referral.managerId).select('name'))?.name || 'Your Manager',
      zoneName: referral.zoneId?.name || '',
      message: `Verification code sent to ${maskPhone(cleanEmail)}. Enter the code to activate your account.`,
    });
  } catch (e) {
...
router.post('/login', async (req, res) => {
  try {
    ...
    if (!id) return res.status(400).json({ message: 'Enter your email or username' });
    ...
    if (!user) return res.status(401).json({ message: 'No account found. Check your email or username.' });
    ...
    res.json({
      ...
      message: `Code sent to ${maskPhone(user.email)}. Enter the code to sign in.`,
    });
  } catch (e) {
